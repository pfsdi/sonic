...
class RetroEnv(gym.Env):
    ...
    def action_to_array(self, a):
        ...
        for p in range(self.players):
            ...
            if self.use_restricted_actions == retro.Actions.DISCRETE:
                for combo in self.action_combos[a[p]]:
                    current = a[p] % len(combo)
                    a //= len(combo)
                    action |= combo[current]
            elif self.use_restricted_actions == retro.Actions.MULTI_DISCRETE:
                if not isinstance(a, (list, np.ndarray)):
                    raise ValueError("Expected action to be a list or array, but got scalar.")
    ...
    def step(self, a):
        ...
        # Ensure that 'a' is a list or array
        if not isinstance(a, (list, np.ndarray)):
            a = [a]
